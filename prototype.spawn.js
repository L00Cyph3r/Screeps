module.exports = function() {
  StructureSpawn.prototype.createCustomCreep = function(energy, roleName) {
    if (roleName == 'harvester' || roleName == 'upgrader' || roleName == 'builder' || roleName == 'repairer' || roleName == 'wallRepairer') {
      ...
      var newCreepName = (
        roleName.substr(0, 1).toUpperCase() + "-" +
        numberOfParts + "-" +
        Math.floor(
        (
          Math.random() * 1000
      ...
      var numberOfParts = Math.floor(energy / (
        150 + 80 + 10 + 10 + 50 + 50
      )); // 350
      var body = [];
      for (let i = 0; i < numberOfParts; i++) {
      ...
      var newCreepName = (
        roleName.substr(0, 1).toUpperCase() + "-" + numberOfParts + "-" + Math.floor((
          Math.random() * 1000
        ) + 1)
      ).toString();
      if (newCreepName) {
        let create = this.createCreep(body, newCreepName, {
          role: roleName,
          mode: 'defending',
